#!/bin/bash
# Verification harness for Server Path.
#
# 1. Runs the ServerPathCore test suite (hermetic, in-process SFTP server).
...
echo "==> [1/2] Running ServerPathCore tests"
(cd "${PROJECT_ROOT}/ServerPathCore" && swift test)

echo "==> [2/2] Building app + File Provider extension"
xcodebuild \
    -project "${PROJECT_ROOT}/Server Path.xcodeproj" \
    -scheme "${SCHEME}" \
    -configuration Debug \
